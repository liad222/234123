...
asmlinkage long sys_wait4(pid_t pid,unsigned int * stat_addr, int options, struct rusage * ru)
{
	...
	if(current->our_policy == 1){
		if(current->our_privilege == 0){
			forbidden_activity_info entry;
			entry.syscall_req_level = 1;
			entry.proc_level = current->our_privilege;
			entry.time = jiffies;
			current->our_log[current->log_index] = entry;
			current->log_index++;
			return -EINVAL;
		}
	}
